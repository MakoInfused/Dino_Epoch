...
void initialize_chr_ram_game(void) {
    ...
    // bank_push(7);
    // vram_adr(PPU_PATTERN_TABLE_1_ADDRESS + 0x080);
    // vram_write((unsigned char*)boss_01, 0x80);
    // vram_adr(PPU_PATTERN_TABLE_1_ADDRESS + 0x180);
    // vram_write((unsigned char*)&boss_01[0x100], 0x80);
    // vram_adr(PPU_PATTERN_TABLE_1_ADDRESS + 0x280);
    // vram_write((unsigned char*)&boss_01[0x200], 0x80);
    // vram_adr(PPU_PATTERN_TABLE_1_ADDRESS + 0x380);
    // vram_write((unsigned char*)&boss_01[0x300], 0x80);
    // bank_pop();
}
    ...
    while (1) {
        ...
        switch (gameState) {
            ...
            case GAME_STATE_POST_TITLE:
                // If A, B and Select are held down as the game is loading, cheat!
                if (pad_state(0) & PAD_SELECT && pad_state(0) & PAD_A && pad_state(0) & PAD_B) {
                    hasSword++;
                    hasShield++;
                    hasSwordUpgrade++;
                    playerMaxHealth += 5;
            ...
            case GAME_STATE_SHOWING_TEXT:
                ...
                gameState = GAME_STATE_RUNNING;
                if(triggered_after_game_text) {
                    banked_call(after_game_text_bank_id, triggered_after_game_text);
                    triggered_after_game_text = NULL;
                    after_game_text_bank_id = 0;
                }
                break;
            ...
            case GAME_STATE_GAME_OVER:
                ...
                fade_in();
                banked_call(PRG_BANK_MENU_INPUT_HELPERS, wait_for_any_key);
                fade_out();
                reset();
                break;
